...
import rednessvalue from '@salesforce/label/c.BI_PSPB_Redness';
...
export default class biPspredness extends NavigationMixin(LightningElement) {
		...
		if (data && data !== null){
			try {
				...
				for (let record of data) {
					...
					if (carePlanTempla === rednessvalue) {
						this.carePlanTemplateName = rednessvalue;
					}
					if (this.carePlanTemplateName === rednessvalue) {
						...
						// Null data is checked and AuraHandledException is thrown from the Apex
							.then(result => {
								if (result && result !== null) {
									...
									sessionStorage.setItem('countred', this.recordInsertCount);
								}
							})
							.catch(error => {
								this.showToast(errormessage, error.message, errorvariant);
