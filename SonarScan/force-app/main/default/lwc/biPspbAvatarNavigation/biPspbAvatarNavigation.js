...
import BI_PSPB_PatientProfileSite from '@salesforce/label/c.BI_PSPB_PatientMyProfileUrl';
import BI_PSPB_PatientNotificationSite from '@salesforce/label/c.BI_PSPB_PatientNotificationUrl';
import BI_PSPB_PatientSelectAvatar from '@salesforce/label/c.BI_PSPB_PatientSelectAvatarUrl';
import BI_PSPB_PatientCaregiver from '@salesforce/label/c.BI_PSPB_MyCaregiverUrl';
import BI_PSPB_BRLetsPersonalizePageTwo from '@salesforce/label/c.BI_PSPB_BRLetsPersonalizePageTwo';
import BI_PSPB_BRLetsPersonalizePageOne from '@salesforce/label/c.BI_PSPB_BRLetsPersonalizePageOne';
...
import BI_PSPB_SymptomTrackerGraph from '@salesforce/label/c.BI_PSPB_symptomtrackergraphUrl'
import BI_PSPB_DlqiHeading from '@salesforce/label/c.BI_PSP_DlqiCategory'
import BI_PSPB_PssHeading from '@salesforce/label/c.BI_PSP_PssCategory'
import BI_PSPB_WapiHeading from '@salesforce/label/c.BI_PSPB_WapiHeading'
...
Use this form to report any..`
	...
	set receivedcategory(value) {
		...
		if (value === BI_PSPB_DlqiHeading) {
			...
			this.contentdot = false;
			if (currentTabName === BI_PSPB_BRSummaryUrl && value === BI_PSPB_DlqiHeading) {
				this.content = false;
...
clicking on the download button.`
		...
		else if (value === BI_PSPB_WapiHeading) {
			this.content = false;
			this.contentdot1 = true;
			this.contentdot = false;
...
clicking on the download button.`
		...
		else if (value === BI_PSPB_PssHeading) {
			this.content = false;
			this.contentdot1 = true;
			this.contentdot = false;
			if (currentTabName === BI_PSPB_BRSummaryUrl) {
...
clicking on the download button.`
	...
	renderedCallback() {
		if (this._receivedcategory === BI_PSPB_DlqiHeading) {
			this.content = false;
			this.contentdot = false;
		}
	...
	wiredavtList({ error, data }) {
		...
		if (data) {
			try {
				...
				//Mobile response
				if (currentTabName === BI_PSPB_BRLetsPersonalizePageOne) {
...
about yourself.`;
				...
				if (currentTabName === BI_PSPB_BRLetsPersonalizePageTwo) {
					this.mobilename = `We want to learn more about you. 
	Personalise your experience by 
	answering a few..`;
					this.selectedName1 = `We want to learn more about you. 
...
of this page.`;
				...
				if (currentTabName === BI_PSPB_SymptomTrackerGraph) {
...
situation.`;
				...
				if (currentTabName === BI_PSPB_PatientProfileSite) {
...
about yourself.`
					...
					else if (currentTabName === BI_PSPB_BRLetsPersonalizePageTwo) {
						this.selectedName1 = `We want to learn more about you. 
Personalise your experience by 
answering a few simple questions 
about yourself.`
					...
					else if (currentTabName === BI_PSPB_PatientProfileSite) {
						this.selectedName1 = `Hi ${this.name}, you're doing great!
We appreciate you sharing, it will help 
us provide you with a better experience 
if we know more about you.
...
information now!`
					...
					else if (currentTabName === BI_PSPB_SymptomTrackerGraph) {
						this.mobilename = `This is your self-symptom tracker, which
	is where you can track..`;
						this.selectedName1 = `This is your self-symptom tracker, which
is where you can track your symptoms,
...
situation.`;
					...
					else if (currentTabName === BI_PSPB_PatientCaregiver) {
...
connected to your healthcare team.`
					...
					else if (currentTabName === BI_PSPB_PatientSelectAvatar) {
						this.mobilename = `Choose your avatar!
	You're about to embark on ...`
						this.selectedName1 = `Choose your avatar!
You're about to embark on a journey 
...
engaging experience along the way. `
					...
					else if (currentTabName === BI_PSPB_PatientNotificationSite) {
						this.mobilename = `Stay in the loop.
	By choosing your notification settings...`
