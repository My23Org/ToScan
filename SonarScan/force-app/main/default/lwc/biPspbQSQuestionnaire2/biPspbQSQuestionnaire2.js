...
import submitAssessmentResponse from '@salesforce/apex/BI_PSP_QsqQuestionnaire.ranswerOfTheQuestionForPageone';
import draftResponseSubmition from '@salesforce/apex/BI_PSP_QsqQuestionnaire.mulitipleDraftRecordsInsertionPageOne';
import draftResponseOfPsoriasis from '@salesforce/apex/BI_PSP_QsqQuestionnaire.draftResponseOfIntroductionPageone';
...
import BI_PSP_Messagepopup from '@salesforce/label/c.BI_PSP_Messagepopup';
import BI_PSP_CompleteAll from '@salesforce/label/c.BI_PSP_CompleteAll';
import BI_PSP_confirmmessage from '@salesforce/label/c.BI_PSP_confirmmessage';
...
export default class BiPspQSQuestionnaire2 extends LightningElement {
	...
	draftSavedMessage = BI_PSP_Messagepopup;
	urlq;
	userid = Id;
	firstDraftResp;
	firstDraftVerionId;
	...
	cannotedit = BI_PSP_confirmmessage;
	...
	reposneModeeOn() {
		...
		const secYes = BI_PSP_yes;
		const secNo = BI_PSP_no;
		...
		//This values are hardcoded because to render the UI/UX design .If custom label's are used the value are not getting populated
		...
		this.responsOfDLQI.forEach((record) => {
			...
			if (record.BI_PSP_ResponseOrder__c === 5) {
				if (
					...
					this.fifthWithoudNewVals = record.ResponseValue;
	...
	closePopup() {
		...
		if (this.sixthQuestionResponse === this.yes) {
			this.seventhYess = true;
		} else if (this.sixthQuestionResponse === this.no) {
			this.seventhNoo = true;
		}
	}
	...
	//Confirm Sumbission List of Response
	confirmSubmission() {
		let firstValue,
			secondValue,
			thirdValue,
