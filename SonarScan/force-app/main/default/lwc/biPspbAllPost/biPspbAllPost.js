...
export default class biPspbAllPost extends (LightningElement) {
		...
		try {
			...
			if (data && data.length === 0) {
				...
				this.allpost = data?.map((post) => ({
					...
					btnchk: post.BI_PSP_FollowStatus__c === following ? following : BI_PSP_Follow,
					youbtn: this.UserAccId === post.BI_PSP_AccountR__c ? false : true,
					postYouName: this.UserAccId === post.BI_PSP_AccountR__c ? You : post.BI_PSP_AccountR__r.BI_PSP_CommunityUsername__c != null ? post.BI_PSP_AccountR__r.BI_PSP_CommunityUsername__c : noUsername,
			...
			emojisave({ reactions: emojis, feedItemId: feedItemId, userId: this.userId })
				.then((result) => {
					...
					if (this.resultvalue && this.resultvalue.length > 0) {
						...
						this.showToast(errormessage, BI_PSP_ChatterUnableToReact, errorvariant); // catch Exception
	...
	Followyes() {
		try {
			this.isLoading = true;
			followuser({ accountIdToFollow: this.acctid, userWhoFollows: this.userId })
				.then(() => {
					this.allpost = this.allpost.map((post) => ({
						...post,
						btnchk:
			...
			unfollowuser({
				...
				.then(() => {
					this.allpost = this.allpost.map((post) => ({
						...post,
						btnchk:
							post.BI_PSP_AccountR__c === this.acctid ? BI_PSP_Follow : post.btnchk
					}));
					this.isLoading = false;
				})
				.catch((error) => {
			...
			followuser({ accountIdToFollow: this.avt, userWhoFollows: this.userId })
				.then(() => {
					this.allpost = this.allpost.map((post) => ({
						...post,
						btnchk:
							post.BI_PSP_AccountR__c === this.acctid || post.BI_PSP_AccountR__c === this.avt ? following : post.btnchk,
					}));
					this.isLoading = false;
					window.scrollTo({ top: 0, behavior: 'smooth' });
				})
	...
	emojiUnFollowingyes() {
		try {
			this.isLoading = true;
			unfollowuser({ accountIdToUnFollow: this.avt, userIdWhoUnFollows: this.userId })
				.then(() => {
					this.allpost = this.allpost.map((post) => ({
						...post,
						btnchk:
							post.BI_PSP_AccountR__c === this.acctid || post.BI_PSP_AccountR__c === this.avt ? BI_PSP_Follow : post.btnchk,
					}));
					this.isLoading = false;
					window.scrollTo({ top: 0, behavior: 'smooth' });
				})
				...
				.then((result) => {
					if (result && result.length > 0) {
						...
						this.displaycomment = result.map((post) => ({
							...
							youname:
								this.UserAccId === post.BI_PSP_AccountCmt__c ? You : post.BI_PSP_AccountCmt__r.BI_PSP_CommunityUsername__c != null
